...
# --- PROCESSAMENTO DE DADOS (INTEGRAL ORIGINAL + ADIÇÃO DE DY E RESOLUÇÃO) ---
tickers_map = {
...
# Carregamento de carteira da memória
...
with tab_painel:
    ...
    # ADIÇÃO: Memória para Capital XP
    ...
    if ativos_sel:
        ...
        for i, nome in enumerate(ativos_sel):
            with cols[i % 2]:
                st.markdown(f"**{nome}**")
                # ADIÇÃO: Memória para Qtd e Investido
                ...
                # Atualizando Session State para salvar depois
                st.session_state.carteira[nome] = {"q": qtd, "i": investido}
...
with tab_radar_modelo:
    ...
    # ADIÇÃO: Colunas DY, LPA e VPA
    html_radar_m = f"""<div class="mobile-table-container"><table class="rockefeller-table">
        <thead><tr><th>Ativo</th><th>Preço (R$)</th><th>Dividendos (DY)</th><th>LPA</th><th>VPA</th><th>Preço Justo</th><th>Status Mercado</th><th>Ação</th></tr></thead>
    ...
    if ativos_sel:
        ...
        if sum(metas.values()) == 100:
            ...
            for nome in ativos_sel:
                # Usa valor atualizado do session state para o plano
                v_at = st.session_state.carteira.get(nome, {}).get("q", 0) * df_radar[df_radar["Ativo"]==nome].iloc[0]["V_Cru"]
...
# ==================== ABA 5: DNA FINANCEIRO (ADIÇÃO DY E RESOLUÇÃO) ====================
with tab_dna:
    st.header("🧬 DNA Financeiro (LPA / VPA)")
    df_combined = pd.concat([df_radar, df_radar_modelo]).drop_duplicates(subset="Ativo")
    # ADIÇÃO: Colunas DY e Resoluções
    html_dna = """<div class="mobile-table-container"><table class="rockefeller-table">
        <thead><tr><th>Ativo</th><th>Dividendos (DY)</th><th>LPA</th><th>Resolução LPA (Lucro/Ações)</th><th>VPA</th><th>Resolução VPA (Patrim./Ações)</th><th>P/L</th><th>P/VP</th></tr></thead><tbody>"""
    for _, r in df_combined.iterrows():
        ...
        html_dna += f"<tr><td>{r['Ativo']}</td><td style='color:#00ff00'>{r['DY']}</td><td>{r['LPA']:.2f}</td><td style='font-size:0.7rem;color:#888'>{res_lpa}</td><td>{r['VPA']:.2f}</td><td style='font-size:0.7rem;color:#888'>{res_vpa}</td><td>{p_l:.2f}</td><td>{p_vp:.2f}</td></tr>"
