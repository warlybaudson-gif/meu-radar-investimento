...
with tab_painel:
    if not df_radar.empty:
        ...
        # CORREÇÃO DO ERRO: Verifica se PETR4.SA existe antes de colocar como padrão
        opcoes = df_radar["Ativo"].unique()
        ...
        if ativos_sel:
            ...
            m2.metric("Troco (XP)", f"R$ {tr_xp:,.2f}")
            m3.metric("PATRIMÔNIO TOTAL", f"R$ {(v_ativos_atualizado + tr_xp):,.2f}")
    else:
        st.error("Falha ao carregar dados. Verifique sua conexão ou tente novamente.")
...
with tab_huli:
    ...
    if 'ativos_sel' in locals() and ativos_sel:
        ...
        st.info("Selecione ativos no Painel de Controle para gerar o plano.")
...
with tab_backtest:
    ...
    if not df_radar.empty:
        ...
        st.success(f"📌 Se você comprasse **{ativo_bt}** no fundo deste mês, seu lucro seria de **{abs(d['Var_Min']):.2f}%** hoje.")
